...
CREATE TABLE p_characters (
    ...
    job_id INTEGER REFERENCES p_jobs (job_id) ON DELETE SET NULL,
    user_id INTEGER REFERENCES p_users (user_id) ON DELETE SET NULL
);
...
-- 演習: ON DELETE SET NULL の動作確認(1)
DELETE FROM p_jobs
WHERE
    job_id = 5;

-- Alice が job_id=5 を参照
SELECT
    *
FROM
    p_characters;

-- 演習: ON DELETE SET NULL の動作確認(2)
DELETE FROM p_users
WHERE
    user_id = 2;
...
-- 更新があると、レコードの順番が変化することにも注意してください。